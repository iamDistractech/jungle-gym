...
import { SESSION_DB_PORT, SESSION_DB_URL, SESSION_DB_PASSWORD, SESSION_DB_USERNAME } from "$lib/Env";
...
if(typeof port === 'number' && !isNaN(port) && typeof host === 'string' ) {
	const options: {host: string, port: number, password: string, username: string, tls: Record<string, unknown>} = { host, port, password: undefined, username: undefined }
